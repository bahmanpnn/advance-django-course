...
class TestBlogView(TestCase):
    ...
    def test_blog_post_detail_anonymouse_response(self):
        ...
        )  # because of login required mixin anonymouse user redirect to login page and status code is 302.

    def test_blog_post_detail_logged_in_user_response(self):
        self.client.force_login(
            self.user
        )  # force login method uses to login user in client and tests.
        url = reverse("blog_module:post-detail", kwargs={"pk": self.post.id})
        response = self.client.get(url)
        self.assertEqual(
