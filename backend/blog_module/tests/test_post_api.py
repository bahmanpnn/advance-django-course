...
def common_user():
    user = User.objects.create_user(
        email="test@test.com", password="Ab12345!@", is_verified=True
    )
    return user
...
class TestBlogPostApi:
    ...
    def test_create_post_response_401_status(self, api_client):
        ...
        url = reverse(
            "blog_module:api-v2:post-model-viewset-list"
        )  # post-model-viewset + list ==> post-model-viewset-list
        data = {
        ...
        )  # status code of response is 401 because user and request need authentication
