    ...
    def validate(self, attrs):
        ...
        if username and password:
            ...
            if not user.is_verified:
                msg = _('Your account is not verified; Unable to log without verifing account.')
                raise serializers.ValidationError(msg, code='authorization')
...
class ChangePasswordSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        if attrs.get('new_password') != attrs.get('new_password1'):
            raise serializers.ValidationError({'detail':'new password and confirm new password don\'t match'})
        try:
            validate_password(attrs.get('new_password')) # django checks password complexity
        except exceptions.ValidationError as e:
